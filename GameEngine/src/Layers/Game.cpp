...
Game::Game()
    : _created_player(0), _player(false), _object(false), _bullet(false) {
}

void Game::initLayer() {
    sf::Color _color(255, 255, 255, 200);
    _manage_elem.createObject<Background>("./Assets/background.jpg", _color, _parallaxShader);
}

void Game::updateLayer(std::vector<std::string> _mapPlayer, std::vector<std::string> _mapObject, std::vector<std::string> _mapBullet, int _id) {
    for (int i = 0; i < _mapPlayer.size(); i++) {
        ...
        if (_created_player != _mapPlayer.size()) {
            ...
            _manage_elem.createEntity<Player>(std::stoi(player_x), std::stoi(player_y), _assets_player[asset_id], Registry &registry);
        } else {
            if (_player == true)
    ...
    for (int i = 0; i < _mapObject.size(); i++) {
        ...
        if (_id == 105) {
            ...
            _manage_elem.createObstacles<Obstacles>(std::stoi(object_x), std::stoi(object_y), _assets_obstacles[random_obstacles], registry);
        } else {
            if (_object == true)
    ...
    for (int i = 0; i < _mapBullet.size(); i++) {
        ...
        if (_id == 107 && i == _mapBullet.size() - 1) {
            _manage_elem.createBullets<Bullet>(std::stoi(object_x), std::stoi(object_y), "./Assets/bullet.gif", registry);
            _bullet = true;
        } else {
            if (_bullet == true)
...
void Game::displayLayer(sf::RenderWindow& _window) {
    _manage_elem.displayObjects(_window, _parallaxShader, _offset, _clockParrallax);
